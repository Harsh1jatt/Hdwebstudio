 ...
 * - Inspects brand intelligence & existing CMS state
 * - Protects against keyword cannibalization
...
import { generateAI } from "./provider.js";
...
const INTENT_PATTERNS = [
  // ─── Create Service ───
  {
    ...
    patterns: [
      /(?:create|add|make|build|generate)\s+(?:a\s+)?(?:new\s+)?(?:service|services?)\s+(?:for|called|named|titled|about|on|targeting|:)\s+(.+)/i,
...
export async function runAgent(userMessage, context = {}) {
  ...
  if (intent.intent === "audit_website") {
    ...
    const msg = `🌐 **Digital Presence Audit Report** for \`${auditReport.domain}\`\n\n` +
  ...
  // ─── 3. Handle Service Creation with Full Pipeline ───
  if (intent.intent === "create_service") {
    const title = intent.params.title;

    // Step 1: Check Brand Context
    steps.push({ tool: "get_brand_context", status: "completed", label: "Loaded HD Web Studios Brand Intelligence" });
    const brand = await getBrandContext();

    // Step 2: Cannibalization & Duplicate Protection
    ...
    // Step 3: AI Content Generation (Brand-Aligned)
    steps.push({ tool: "generate_service_content", status: "executing", label: "Generating Structured Service Architecture" });
    const prompt = `Generate a professional service page for "${title}" for ${brand.brandName} (${brand.primaryGeography.city}, ${brand.primaryGeography.state}, ${brand.primaryGeography.country}). Positioning: ${brand.positioning}. Return ONLY JSON with fields: title, tagline, description, shortDescription, category, whatYouGet (4-6 items with icon, title, text), faq (3-4 items with q, a), seoTitle, seoDescription.`;
    const genResult = await generateAI(prompt, { contentType: "service" });
    let serviceData;
    try {
      serviceData = JSON.parse(genResult.text);
    } catch {
      serviceData = {
        title,
        tagline: `Professional ${title} solutions by HD Web Studios in Ludhiana, Punjab.`,
        description: `High-performance ${title} services engineered for businesses to acquire customers online.`,
      };
    }
    steps[2].status = "completed";

    // Step 4: Deterministic SEO Evaluation & Multi-Pass Optimization Loop
    steps.push({ tool: "analyze_seo", status: "executing", label: "Running 100-Point SEO Intelligence Evaluation" });
    let seoEval = evaluateSEO(serviceData, "service");
    steps[3].status = "completed";

    let iteration = 0;
    const MAX_ITERATIONS = 2;
    while (seoEval.score < 85 && iteration < MAX_ITERATIONS) {
      iteration++;
      steps.push({
        tool: "optimize_seo_content",
        status: "executing",
        label: `SEO Optimization Loop (Pass ${iteration}): Improving weaknesses (Score: ${seoEval.score}/100)`,
      });

      if (!serviceData.focusKeyword) {
        serviceData.focusKeyword = serviceData.title.toLowerCase();
      }

      if (seoEval.breakdown.metadata < 15) {
        serviceData.seoTitle = `${serviceData.title} in Ludhiana | HD Web Studios`;
        serviceData.seoDescription = `Professional ${serviceData.title.toLowerCase()} in Ludhiana, Punjab. High-performance, mobile-first web engineering to grow your business inquiries.`;
      }

      if (!serviceData.faq || serviceData.faq.length < 3) {
        serviceData.faq = [
          { q: `How long does ${serviceData.title.toLowerCase()} take?`, a: "Standard delivery is between 7 to 14 business days from kickoff." },
          { q: `Do you provide SEO with ${serviceData.title.toLowerCase()}?`, a: "Yes, on-page optimization, LocalBusiness schema, and Core Web Vitals speed tuning are included." },
          { q: "Do I own 100% of the website code and assets?", a: "Yes, you retain full ownership of all source code, domain assets, and database records." },
        ];
      }

      seoEval = evaluateSEO(serviceData, "service");
    ...
    const saveResult = await TOOLS.create_service.execute(
      { ...serviceData, publish: brand.aiAutoPublish },
    ...
    if (!saveResult.success) {
      return {
        state: "failed",
        message: `❌ **Failed to create service:** ${saveResult.message}`,
        steps,
        actions: [],
      };
    }
    ...
    const message = `✅ **Service Created Successfully (Draft Mode)**\n\n` +
      `• **Title:** ${serviceData.title}\n` +
      `• **Slug:** \`${saveResult.slug}\`\n` +
      `• **SEO Score:** **${seoEval.score}/100** (${seoEval.grade})\n` +
      `• **Technical:** ${seoEval.breakdown.technical}/20 | **Metadata:** ${seoEval.breakdown.metadata}/15 | **Keywords:** ${seoEval.breakdown.keywords}/15\n` +
    ...
    return {
      ...
      steps,
      actions: [
        { label: "Review & Publish", href: saveResult.editUrl },
  ...
  if (intent.intent === "create_blog") {
    const title = intent.params.title;
    steps.push({ tool: "get_brand_context", status: "completed", label: "Loaded Brand Voice" });
    const brand = await getBrandContext();
    ...
    const prompt = `Write an in-depth, commercial-focused blog article titled "${title}" for ${brand.brandName} (${brand.primaryGeography.city}, ${brand.primaryGeography.state}). Return ONLY JSON with fields: title, excerpt, content (HTML with h2, h3, p, ul, strong), category, tags, seoTitle, seoDescription, focusKeyword.`;
    const gen = await generateAI(prompt, { contentType: "blog" });
    let blogData;
    try {
      blogData = JSON.parse(gen.text);
    } catch {
      blogData = { title, excerpt: `Guide to ${title}.`, content: `<p>Comprehensive guide to ${title}.</p>` };
    }
    steps[2].status = "completed";

    // Step 4: Deterministic SEO Evaluation & Multi-Pass Optimization Loop
    steps.push({ tool: "analyze_seo", status: "executing", label: "Running 100-Point SEO Intelligence Evaluation" });
    let seoEval = evaluateSEO(blogData, "blog");
    steps[3].status = "completed";
    ...
    while (seoEval.score < 85 && iteration < MAX_ITERATIONS) {
      ...
      steps.push({
        tool: "optimize_seo_content",
        status: "executing",
        label: `SEO Optimization Loop (Pass ${iteration}): Refining article structure & keywords (Score: ${seoEval.score}/100)`,
      });

      if (!blogData.focusKeyword) {
        blogData.focusKeyword = blogData.title.toLowerCase();
      }

      if (seoEval.breakdown.metadata < 15) {
        blogData.seoTitle = `${blogData.title} | HD Web Studios`;
        blogData.seoDescription = `Learn all about ${blogData.title.toLowerCase()}. Practical strategies and business guidance by HD Web Studios in Ludhiana, Punjab.`;
      }

      seoEval = evaluateSEO(blogData, "blog");
      steps[steps.length - 1].status = "completed";
    }
    ...
    const saveResult = await TOOLS.create_blog.execute(
      { ...blogData, publish: brand.aiAutoPublish },
      { adminId: context.adminId, adminName: context.adminName, prompt: userMessage }
    );
    steps[steps.length - 1].status = saveResult.success ? "completed" : "failed";
    ...
    return {
      state: "completed",
      message: `✅ **Blog Article Created (Draft Mode)**\n\n` +
        `• **Title:** ${blogData.title}\n` +
        `• **Slug:** \`${saveResult.slug}\`\n` +
        `• **SEO Score:** **${seoEval.score}/100** (${seoEval.grade})\n` +
        `• **Reading Time:** ~${blogData.readingTime || 3} min\n` +
        `• **Status:** Draft\n\n` +
  ...
  if (intent.intent === "audit_summary") {
    ...
    return {
      state: "completed",
      message: `📊 **HD Web Studios CMS & Content Status**\n\n` +
  ...
  if (intent.intent === "get_content_clusters") {
    ...
    const clusterList = clusterResult.clusters.map((c) =>
  ...
  if (intent.tool && TOOLS[intent.tool]) {
    ...
    const result = await tool.execute(intent.params, { adminId: context.adminId, adminName: context.adminName, prompt: userMessage });
  ...
  const aiRes = await generateAI(assistantPrompt);
