...
import { teamMemberPayloadSchema } from "@/utils/teamValidation";
...
export async function PUT(req, { params }) {
  ...
  const result = teamMemberPayloadSchema.safeParse(body);
  if (!result.success) return NextResponse.json({ success: false, error: "Validation failed", details: result.error.flatten() }, { status: 400 });
  const doc = await TeamMember.findByIdAndUpdate(id, result.data, { new: true, runValidators: true }).lean();
  if (!doc) return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  return NextResponse.json({ success: true, item: { ...doc, id: doc._id.toString(), _id: undefined } });
}
...
export async function PATCH(req, { params }) {
  ...
  const doc = await TeamMember.findByIdAndUpdate(id, allowed, { new: true }).lean();
  if (!doc) return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  return NextResponse.json({ success: true, item: { ...doc, id: doc._id.toString(), _id: undefined } });
}
