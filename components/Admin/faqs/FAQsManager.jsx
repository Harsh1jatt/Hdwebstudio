...
export default function FAQsManager() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const fetchedItems = data.items || data.faqs || [];
      setItems(fetchedItems);
      setTotal(data.total || 0);
      setPage(data.page || page);
      const uniqueCats = [...new Set(fetchedItems.map((i) => i.category).filter(Boolean))];
      setCategories((prev) => {
  ...
  async function handleToggle(item, field) {
    try {
      const res = await fetch(`/api/admin/faqs/${item._id || item.id}`, {
        method: "PATCH",
  ...
  async function handleDelete(item) {
    ...
    try {
      const res = await fetch(`/api/admin/faqs/${item._id || item.id}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Delete failed.");
      setActionMessage("FAQ deleted.");
      await fetchItems();
    } catch (err) {
      alert(err.message);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-blue-600">FAQs CMS</p>
          <h2 className="mt-2 text-2xl font-semibold text-slate-900">Manage FAQs</h2>
          <p className="mt-2 text-sm text-slate-500">Create, edit, and publish frequently asked questions.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-[1fr_auto_auto]">
        ...
        />
        <div>
          <label className="mb-2 block text-[13px] font-semibold text-slate-700">Category</label>
          <select
            value={categoryFilter}
            onChange={(e) => { setCategoryFilter(e.target.value); setPage(1); }}
            className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-sm text-slate-900 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 md:min-w-[160px]"
          >
      ...
      {!loading && !error && items.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center">
          <h3 className="text-lg font-semibold text-slate-900">No FAQs yet</h3>
          <p className="mt-2 text-sm text-slate-500">Create your first FAQ to get started.</p>
          <div className="mt-6">
            <Link href="/admin/faqs/new"><AdminButton>Create FAQ</AdminButton></Link>
          </div>
        </div>
      ) : null}
      ...
      {!loading && items.length > 0 ? (
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item._id || item.id} className="border-b border-slate-100">
                    <td className="max-w-xs truncate px-4 py-3 font-medium text-slate-900">{item.question}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap items-center gap-2">
                        <Link href={`/admin/faqs/${item._id || item.id}`} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">
          ...
          <div className="flex items-center justify-between border-t border-slate-200 px-4 py-3 text-sm text-slate-500">
            <div>{Math.min(1 + (page - 1) * defaultPerPage, total)} - {Math.min(page * defaultPerPage, total)} of {total} FAQs</div>
